...
net.Receive("Horde_BuyItem", function (len, ply)
    ...
    if ply:Horde_GetMoney() >= price and ply:Horde_GetWeight() >= weight and ply:Horde_GetSkullTokens() >= skull_tokens and level_satisfy then
        ...
        net.WriteString(translate.ClientFormat(ply, "Notification_You_Bought_X", translate.ClientGet(ply, "Item_" .. item.name)))
        net.WriteInt(0,2)
        net.Send(ply)
        ply:Horde_SyncEconomy()
    end
...
net.Receive("Horde_SellItem", function (len, ply)
    ...
    if canSell == false then
        net.Start("Horde_LegacyNotification")
        net.WriteString(why or translate.ClientGet(ply, "Notification_You_Cant_Sell_This"))
        net.WriteInt(1,2)
        net.Send(ply)
        return
    end
...
net.Receive("Horde_SelectClass", function (len, ply)
    ...
    if ply:Alive() then
        if HORDE.start_game and HORDE.current_break_time <= 0 then
            net.Start("Horde_LegacyNotification")
            net.WriteString(translate.ClientGet(ply, "Notification_You_Cannot_Change_Class_After_A_Wave_Has_Started"))
            net.WriteInt(1,2)
            net.Send(ply)
            return
        end
        if GetConVar("horde_testing_unlimited_class_change"):GetInt() == 0 and HORDE.player_class_changed[ply:SteamID()] then
            net.Start("Horde_LegacyNotification")
            net.WriteString(translate.ClientGet(ply, "Notification_You_Cannot_Change_Class_More_Than_Once_Per_Wave"))
            net.WriteInt(1,2)
            net.Send(ply)
            return
        end
    ...
    if ply:Horde_GetSubclassUnlocked(subclass_name) == false then
        net.Start("Horde_LegacyNotification")
        net.WriteString(translate.ClientFormat(ply, "Notification_Subclass_X_Is_Not_Unlocked_On_This_Server", translate.ClientGet(ply, "Subclass_Title_" .. subclass_name)))
        net.WriteInt(1,2)
        net.Send(ply)
        return
    end
    ...
    net.Start("Horde_LegacyNotification")
    net.WriteString(translate.ClientFormat(ply, "Notification_Changed_Class_To_X", translate.ClientGet(ply, "Class_" .. class.name)))
    net.WriteInt(0,2)
    net.Send(ply)
...
net.Receive("Horde_BuyItemAmmoPrimary", function (len, ply)
    ...
    if not ply:HasWeapon(class) then
        net.Start("Horde_LegacyNotification")
        net.WriteString(translate.ClientGet(ply, "Notification_You_Dont_Have_This_Weapon"))
        net.WriteInt(0,2)
        net.Send(ply)
        return
    end
