...
hook.Add("PlayerSpawn", "Horde_PlayerSpawnMidWave", function (ply)
    if HORDE.start_game and HORDE.current_break_time <= 0 then
        if ply:IsValid() then
            ply:KillSilent()
            net.Start("Horde_LegacyNotification")
            net.WriteString(translate.ClientGet(ply, "Notification_You_Will_Respawn_Next_Wave"))
            net.Send(ply)
        end
    end
end)
...
hook.Add("DoPlayerDeath", "Horde_DoPlayerDeath", function(victim)
    ...
    net.Start("Horde_LegacyNotification")
    net.WriteString(translate.ClientGet(victim, "Notification_You_Are_Dead_You_Will_Respawn_Next_Wave"))
    net.Send(victim)
