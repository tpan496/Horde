...
if SERVER then
    ...
    net.Receive("Horde_UnlockSubclass", function (len, ply)
        ...
        if ply:Horde_GetSubclassUnlocked(subclass_name) == true then
            ...
            net.WriteString(translate.ClientFormat(ply, "Notification_Subclass_X_Already_Unlocked", translate.ClientGet(ply, subclass.PrintName)))
            net.WriteInt(1,2)
            net.Send(ply)
            return
        end
        ...
        if ply:Horde_GetSkullTokens() >= cost then
            ...
            net.Start("Horde_LegacyNotification")
            net.WriteString(translate.ClientFormat(ply, "Notification_You_Unlocked_X_Subclass", translate.ClientGet(ply, subclass.PrintName)))
            net.WriteInt(0,2)
            net.Send(ply)
        end
    end)
...
function plymeta:Horde_SetSubclass(class_name, subclass_name)
    ...
    if SERVER then
        net.Start("Horde_LegacyNotification")
            net.WriteString(translate.ClientFormat(self, "Notification_X_Subclass_Changed_To_X", translate.ClientGet(self, "Subclass_Title_" .. class_name), translate.ClientGet(self, "Subclass_Title_" .. HORDE.subclasses[subclass_name].PrintName)))
            net.WriteInt(0,2)
        net.Send(self)
    end
    if CLIENT then
