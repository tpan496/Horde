    ...
    if took then
                ...
                if GetConVar("horde_testing_display_damage"):GetInt() == 1 then
                    ...
                    net.Start("Horde_LegacyNotification")
                        if dmgtype == HORDE.DMG_PURE then
                            net.WriteString(translate.ClientFormat(dmg:GetAttacker(), "Notification_You_Dealth_X_Damage_To_X", dmg:GetDamage(), ent:GetClass()))
                            net.WriteInt(0,2)
                        else
                            net.WriteString(translate.ClientFormat(dmg:GetAttacker(), "Notification_You_Dealth_X_X_Damage_To_X", dmg:GetDamage(), translate.ClientGet(dmg:GetAttacker(), "Damage_Type_" .. HORDE.DMG_TYPE_STRING[dmgtype]), ent:GetClass()))
                            net.WriteInt(0,2)
                        end
        ...
        elseif ent:IsPlayer() and dmg:GetAttacker():IsNPC() then
            ...
            if GetConVar("horde_testing_display_damage"):GetInt() == 1 then
                ...
                net.Start("Horde_LegacyNotification")
                    if dmgtype == HORDE.DMG_PURE then
                        net.WriteString(translate.ClientFormat(ent, "Notification_You_Received_X_Damage_From_X", dmg:GetDamage(), dmg:GetAttacker():GetClass()))
                        net.WriteInt(0,2)
                    else
                        net.WriteString(translate.ClientFormat(ent, "Notification_You_Received_X_X_Damage_From_X", dmg:GetDamage(), HORDE.DMG_TYPE_STRING[dmgtype], dmg:GetAttacker():GetClass()))
                        net.WriteInt(0,2)
                    end
                net.Send(ent)
            end
