...
function Start(ply)
    if ply:IsAdmin() then
        ...
        net.WriteString(translate.ClientGet(ply, "Notification_You_Do_Not_Have_Access_To_This_Command"))
        net.WriteInt(1,2)
        net.Send(ply)
        return
    end
...
function Ready(ply)
    ...
    if not ply:Alive() then
        net.Start("Horde_LegacyNotification")
        net.WriteString(translate.ClientGet(ply, "Notification_You_Cant_Get_Ready_When_You_Are_Dead"))
        net.WriteInt(1,2)
        net.Send(ply)
        return
    end
...
function Shop(ply)
    if GetConVar("horde_enable_shop"):GetInt() == 0 then
        net.Start("Horde_LegacyNotification")
        net.WriteString(translate.ClientGet(ply, "Notification_Shop_Has_Been_Disabled"))
        net.WriteInt(1,2)
        net.Send(ply)
    end
    --[[if not ply:Alive() then
        net.Start("Horde_LegacyNotification")
        net.WriteString(translate.ClientGet(ply, "Notification_You_Cant_Buy_When_Dead"))
        net.WriteInt(1,2)
        net.Send(ply)
    ...
    if HORDE.current_break_time <= 0 then
        net.Start("Horde_LegacyNotification")
        net.WriteString(translate.ClientGet(ply, "Notification_You_Cannot_Shop_After_A_Wave_Has_Started"))
        net.WriteInt(1,2)
        net.Send(ply)
        return
    end]]--
...
function ItemConfig(ply)
    if HORDE.start_game then
        net.Start("Horde_LegacyNotification")
        net.WriteString(translate.ClientGet(ply, "Notification_You_Cannot_Open_Config_After_A_Game_Has_Started"))
        net.WriteInt(1,2)
        net.Send(ply)
        return
    end
...
concommand.Add("horde_testing_gorlami", function (ply, cmd, args)
    if GetConVar("horde_enable_sandbox"):GetInt() == 0 then
        net.Start("Horde_LegacyNotification")
            net.WriteString(translate.ClientGet(ply, "Notification_Command_Only_Available_In_Sandbox_Mode"))
            net.WriteInt(1,2)
        net.Send(ply)
        return
    end
