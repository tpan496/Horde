...
PERK.Hooks.M_Hook_Mult_ReloadTime = function(wep, data)
    local ply = wep:GetOwner()
    if IsValid(ply) and ply:IsPlayer() and ply:Horde_GetPerk("arccw_mg_reload")
            and (wep.RegularClipSize or wep.Primary.ClipSize) >= ply:Horde_GetPerkParam("arccw_mg_reload", "threshold") then
