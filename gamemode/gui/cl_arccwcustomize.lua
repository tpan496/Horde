...
local buyMaterial = Material("status/canbuy_white.png", "smooth")
...
function meta:ShowInventoryButton()
...
function meta:GetSlotInstalled(i)
...
surface.CreateFont("ARCCW_TOOLTIP", {
    font = "Arial",
    extended = false,
 ...
 local getTextSize = function(font, text)
     surface.SetFont(font)
    return surface.GetTextSize(text)
end

function meta:CreateCustomize2HUD()
    ...
    ArcCW.InvHUD.ToolTipAlpha = 0
    ArcCW.InvHUD.ToolTip = "Hello World!"
    ArcCW.InvHUD.ToolTipTime = 0

    local tooltipLayer = vgui.Create("DImage", ArcCW.InvHUD) -- DImage doesn't fuck with inputs
    tooltipLayer:SetSize(scrw, scrh)
    tooltipLayer.Paint = function()
        if(ArcCW.InvHUD.ToolTipTime > SysTime()) then
            ArcCW.InvHUD.ToolTipAlpha = math.Clamp(ArcCW.InvHUD.ToolTipAlpha + 700 * RealFrameTime(), 0, 255)
        else
            ArcCW.InvHUD.ToolTipAlpha = math.Clamp(ArcCW.InvHUD.ToolTipAlpha - 700 * RealFrameTime(), 0, 255)
        end

        if(ArcCW.InvHUD.ToolTipAlpha > 0) then
            local x, y = gui.MousePos()
            local w, h = getTextSize("ARCCW_TOOLTIP", ArcCW.InvHUD.ToolTip)
            local padding = ScreenScaleH(2)
            y = y + padding * 5
            draw.RoundedBox(0, x - (padding + w * 0.5), y - padding, w + padding * 2, h + padding * 2, Color(0, 0, 0, ArcCW.InvHUD.ToolTipAlpha * 0.5))
            draw.DrawText(ArcCW.InvHUD.ToolTip, "ARCCW_TOOLTIP", x, y, Color(255, 255, 255, ArcCW.InvHUD.ToolTipAlpha), TEXT_ALIGN_CENTER)
        end
    end
    tooltipLayer:SetZPos(32767)

    ArcCW.InvHUD.UpdateToolTip = function(tip, time)
        ArcCW.InvHUD.ToolTip = tip
        ArcCW.InvHUD.ToolTipTime = SysTime() + time
    end
    ...
    local function FixedValue(input)
        local target = 0.016666
        return input / (target / RealFrameTime())
    end

    --[[
    local function PopupBuyMenu(parent, w, h, slot, class, price)
        local popup = vgui.Create("DPanel")
        popup:MakePopup()
        popup:SetSize(w, h)
        popup.CWide = 1
        popup.FirstFrame = translate

        local mdown = false
        local canbuy = LocalPlayer():Horde_GetMoney() >= HORDE.GetAttachmentPrice(slotName, class)
        popup.Think = function()
            if(popup.FirstFrame) then popup.FirstFrame = false return end
            local d = input.IsMouseDown(107)
            local x, y = input.GetCursorPos()
            if(d && mdown && (x < popup:GetX() || x > popup:GetX() + popup:GetWide() || y < popup:GetY() || y > popup:GetY() + popup:GetTall())) then -- HasFocus() doesn't work, had to check it manually
                popup:Remove()
            end
            mdown = d
            if(!popup:HasFocus() || !parent:IsVisible(false)) then
                popup:Remove()
                return
            end
            popup:SetPos(parent:LocalToScreen(parent:GetWide() - (parent.CornerRadius || 0) * 0.5, 0))
        end

        popup.Alpha = 0
        popup.Paint = function()
            popup.Alpha = math.Clamp(popup.Alpha + FixedValue(20), 0, 255)
            popup.CWide = math.Clamp(popup.CWide + FixedValue((w - popup.CWide) * 0.2), 0, w)
            if(canbuy) then
                draw.RoundedBox(0, 0, 0, popup.CWide, popup:GetTall(), Color(30, 30, 30, popup.Alpha))
            else
                draw.RoundedBox(0, 0, 0, popup.CWide, popup:GetTall(), Color(100, 30, 30, popup.Alpha))
            end
            draw.DrawText("BUY ("..price.."$)", "ArcCWC2_8", w * 0.5, h * 0.25, Color(255, 255, 255, popup.Alpha), TEXT_ALIGN_CENTER)
        end

        function popup:OnMousePressed()
            if(!canbuy) then return end
            net.Start("Horde_BuyAttachment")
    ...
    function ArcCW.InvHUD_FormAttachmentSelect()
        ...
        local attCheck = {}
        local slotName = slot.PrintName || "NULL"
        ...
        local has = false
        local buyButtonSize = smallbuttonheight
        local dvbar = ArcCW.InvHUD_Menu2:GetVBar()
        for _, att in pairs(atts) do
            if !att then continue end
            if !istable(att) then continue end
            local ind = _
            local class = att.att
            local attachmentscheck = ArcCW.AttachmentTable[class]
            if(_ != 0 && !HORDE.IsAttachmentWhitelisted(class)) && !attachmentscheck.Free && GetConVar("horde_arccw_attinv_free"):GetInt() == 0 then continue end
            ...
            local basepnl = vgui.Create("DPanel", ArcCW.InvHUD_Menu2)
            basepnl:DockMargin(0, smallgap, 0, 0)
            basepnl:Dock(TOP)
            basepnl:SetSize(menu2_w - (2 * ss), smallbuttonheight)
            basepnl.Paint = function()
            end

            if(ind != 0 && !attachmentscheck.Free && GetConVar("horde_arccw_attinv_free"):GetInt() == 0) then
                local canbuy = false
                local intrader = false
                local buy = vgui.Create("DButton", basepnl)
                buy:SetText("")
                buy:SetSize(buyButtonSize, buyButtonSize)
                buy:SetPos(basepnl:GetWide() - (buyButtonSize + smallgap), 0)
                buy.Alpha = 0
                buy.Paint = function()
                    canbuy = LocalPlayer():Horde_GetMoney() >= HORDE.GetAttachmentPrice(slotName, class)
                    intrader = LocalPlayer().Horde_CanBuy
                    if(buy:IsHovered() || IsValid(buy.popup)) then
                        buy.Alpha = math.Clamp(buy.Alpha + FixedValue(10), 0, 100)
                        ArcCW.InvHUD.UpdateToolTip("BUY ("..HORDE.GetAttachmentPrice(slotName, class).."$)", 1)
                    else
                        buy.Alpha = math.Clamp(buy.Alpha - FixedValue(10), 0, 100)
                    end
                    surface.SetDrawColor(255, 255, 255, 255)
                    surface.SetMaterial(buyMaterial)
                    surface.DrawTexturedRect(0, 0, buyButtonSize, buyButtonSize)
                    local noattwl = HORDE.items[class]
                    if noattwl == nil then
                        draw.RoundedBox(cornerrad, 0, 0, buyButtonSize, buyButtonSize, Color(255, 30, 30, 200))
                    elseif(canbuy) and intrader then
                        draw.RoundedBox(cornerrad, 0, 0, buyButtonSize, buyButtonSize, Color(30, 30, 30, 200))
                    else
                        draw.RoundedBox(cornerrad, 0, 0, buyButtonSize, buyButtonSize, Color(150, 30, 30, 200))
                    end
                    draw.RoundedBox(cornerrad, 0, 0, buyButtonSize, buyButtonSize, Color(255, 255, 255, buy.Alpha))
                end
                buy.CornerRadius = cornerrad
                buy.DoClick = function()
                    surface.PlaySound("UI/buttonclick.wav")
                    if(!canbuy) or !intrader then return end
                    net.Start("Horde_BuyAttachment")
            ...
            local button = vgui.Create("DButton", basepnl)
            button.att = att.att
            button.attslot = att.slot
            button:SetText("")
            if GetConVar("horde_arccw_attinv_free"):GetInt() == 0 then
                button:SetSize(basepnl:GetWide() - (buyButtonSize + smallgap * 2), basepnl:GetTall())
            else
                button:SetSize(basepnl:GetWide() - (smallgap * 2), basepnl:GetTall())
            end
            button.DoClick = function(self2, clr, btn)
            ...
            button.DoRightClick = function(self2)
                if(GetConVar("horde_testing_attachment_copy"):GetInt() == 1) then
                    HORDE:PlayNotification("Class copied ("..class..")", type, "ok.png")
