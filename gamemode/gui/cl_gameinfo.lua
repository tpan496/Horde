    ...
    corner_panel.Paint = function ()
        ...
        if MySelf:Alive() then
            if (HORDE.current_wave <= 0) or (wave_str == nil) then
                ...
                draw.SimpleText(translate.Get("Game_WAVE") .. wave_str, "Info", ScreenScale(45), ScreenScale(7), Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            end
        else
            draw.SimpleText(translate.Get("Game_Spectate"), "Info", ScreenScale(45), ScreenScale(7), Color(255,255,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
