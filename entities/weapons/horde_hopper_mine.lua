...
if (CLIENT) then
	SWEP.WepSelectIcon = surface.GetTextureID("vgui/hud/horde_hopper_mine")
    SWEP.DrawWeaponInfoBox	= false
    SWEP.BounceWeaponIcon = false
	killicon.Add("horde_hopper_mine", "vgui/hud/horde_hopper_mine", color_white)
    killicon.Add("horde_mine_hopper", "vgui/hud/horde_hopper_mine", color_white)
end
...
function SWEP:PrimaryAttack()
	...
	if (self.Owner:GetAmmoCount("horde_mine") > 0 and self:GetNextPrimaryFire() <= CurTime()) then
		...
		if not self.Owner.Horde_Hopper_Mines then
			self.Owner.Horde_Hopper_Mines = HORDE.Queue:Create()
		end

		if self.Owner.Horde_Hopper_Mines:size() >= 5 then
			local s = self.Owner.Horde_Hopper_Mines:pop()
			while !IsValid(s) and self.Owner.Horde_Hopper_Mines:size() > 0 do
				s = self.Owner.Horde_Hopper_Mines:pop()
			end
			if IsValid(s) then
				s:Remove()
			end
		...
		timer.Simple(0.5, function ()
			...
			local ent = ents.Create("horde_mine_hopper")
			...
			self.Owner.Horde_Hopper_Mines:push(ent)
